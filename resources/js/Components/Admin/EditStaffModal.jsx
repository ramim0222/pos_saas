...
export default function EditStaffModal({ open, onClose, staff, branches }) {
    ...
    const submit = (e) => {
        ...
        router.put(
            route("admin.staff.role", staff.id),
            { role },
            {
                preserveScroll: true,
                onSuccess: () => {
                    router.put(
                        route("admin.staff.branches", staff.id),
        ...
        <div ref={rootRef} className="fixed inset-0 z-50 flex">
            ...
            <div ref={panelRef} className="relative ml-auto flex h-full w-full max-w-md flex-col bg-front-bg shadow-2xl">
                <div className="flex shrink-0 items-center justify-between border-b border-front-line px-6 py-5">
                    <h2 className="font-display text-xl font-medium text-front-ink">Edit {staff.name}</h2>
                    <button
                        type="button"
                        onClick={onClose}
                ...
                <form id="edit-staff-form" onSubmit={submit} noValidate className="flex-1 space-y-5 overflow-y-auto px-6 py-6">
                    <div>
                ...
                <div className="flex shrink-0 items-center justify-end gap-3 border-t border-front-line px-6 py-4">
                    ...
                    <FrontButton type="submit" form="edit-staff-form" disabled={processing}>
                        {processing ? (
                            <>
