...
export default function BranchesPageHeader({ usage, onAdd }) {
    ...
    return (
        <div ref={rootRef} className="space-y-4">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    <h1 className="font-display text-2xl font-semibold text-front-ink">Branches</h1>
                    <p className="mt-1 text-sm text-front-muted">Manage your store's physical locations and warehouses</p>
                </div>
                <button
                    type="button"
                    onClick={onAdd}
            ...
            <div className="rounded-xl border border-front-line bg-front-surface px-5 py-4">
                <div className="flex items-center justify-between text-sm">
                    <span className="text-front-ink/85">
                        {usage.limit === null ? (
                            <>
                                {usage.used} branch{usage.used === 1 ? "" : "es"} · <span className="text-front-muted">Unlimited on {usage.plan}</span>
                            </>
                        ) : (
                            <>
                                {usage.used} of {usage.limit} branches used
                            </>
                        )}
                    </span>
                ...
                {atLimit && (
                    <div className="mt-3 flex items-start gap-2 rounded-lg border border-front-accent/30 bg-front-accent/[0.06] px-3.5 py-2.5 text-xs text-front-ink/80">
                        ...
                        <span>
                            You've reached your {usage.plan} plan's branch limit. Head to{" "}
                            <a href={route("admin.billing.index")} className="font-medium text-front-accent hover:underline">
                                Billing
                            </a>{" "}
                            to upgrade and add more locations.
                        </span>
                    </div>
                )}
            </div>
